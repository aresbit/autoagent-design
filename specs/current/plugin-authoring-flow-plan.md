...
- R2. The create entry navigates to Home, focuses `HomeHero`'s textarea, and pre-fills a prompt grounded in the Auto Design plugin spec.
...
- No new plugin spec dialect. The generated folder must validate against the existing Auto Design plugin shape in `docs/plugins-spec.md` and `docs/schemas/open-design.plugin.v1.json`.
...
- [x] U3. **Plugin authoring scenario**
  ...
  **Approach:**
  ...
  - The prompt should cite the Auto Design plugin spec and require validation-ready output, not a prose-only explanation.
...
```text
Create an Auto Design plugin for: <user goal>.
