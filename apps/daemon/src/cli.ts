...
// external agents (hermes-agent, openclaw, etc.) can drive Auto Design
...
function printRootHelp() {
  ...
  od mcp [--daemon-url <url>]
      ...
      running Auto Design daemon. Wire it into a coding agent
      (Claude Code, Cursor, VS Code, Zed, Windsurf) in another repo
      to pull files from a local Auto Design project and create
...
Runs Tavily-backed shallow research through the local Auto Design daemon.
  ...
  if (code === 'EPERM' || code === 'ENETUNREACH') {
    console.error(
      'hint: outbound connect was denied by a sandbox. If you launched ' +
        ...
        'network policy. The Auto Design daemon itself is unaffected - it can be ' +
...
tool calls to a running Auto Design daemon. Wire it into a coding agent
in another repo so the agent can pull files from a local Auto Design
project and create project-scoped artifacts without exporting a zip
every iteration.

Options:
  --daemon-url <url>   Auto Design daemon HTTP base URL. Resolution
...
Tools exposed:
  list_projects                  list every Auto Design project
...
user has open in Auto Design; get_artifact and get_file additionally
...
→ MCP server in the Auto Design app. The daemon must be running locally
...
Wraps GitHub CLI auth for Auto Design registry publishing. The token stays in gh.`);
    return;
  }
...
Shows the GitHub account gh will use for Auto Design registry publishing.`);
    return;
  }
  const host = typeof flags.host === 'string' ? flags.host : 'github.com';
...
Common options:
  --daemon-url <url>   Auto Design daemon HTTP base (default OD_DAEMON_URL, OD_SIDECAR_IPC_PATH discovery, or http://127.0.0.1:7456).
  ...
  switch (sub) {
    ...
    case 'login': {
      ...
      console.log(`[marketplace login] authenticating gh for ${host}. Tokens stay in gh, not Auto Design.`);
      const result = await spawnPassthrough('gh', ['auth', 'login', '--hostname', host, '--web']);
      process.exit(result.code ?? 0);
    }
    case 'add': {
...
marks a version unresolvable for new installs while preserving lockfile replay.`);
  ...
  const payload = {
    ...
    version: parsed.range,
    reason,
    url: `https://github.com/aresbit/autoagent-design/issues/new?${params.toString()}`,
    body,
  };
  if (flags.json) {
    process.stdout.write(JSON.stringify(payload, null, 2) + '\n');
...
// these so a code agent can drive Auto Design end-to-end without
...
Common options:
  --daemon-url <url>   Auto Design daemon HTTP base.
  --json               Emit raw JSON.`);
    process.exit(args.length === 0 ? 2 : 0);
  }
  const sub = args[0];
