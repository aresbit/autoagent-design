...
//   - open-design            → aresbit/autoagent-design (plugins/community/<plugin-name>/).
...
export function buildPublishLink(args: {
  ...
  switch (args.catalog) {
    ...
    case 'open-design': {
      ...
      const url = newIssueUrl('aresbit/autoagent-design', title, bodyWithRegistry);
      return {
        catalog: args.catalog,
        catalogLabel: 'aresbit/autoagent-design',
        url,
...
function renderPrBody(m: PublishMetadata): string {
  ...
  lines.push('- Ships `open-design.json` sidecar (additive Auto Design metadata).');
  lines.push('');
