...
describe('buildPublishLink', () => {
  ...
  it('builds an Auto Design registry submission URL', () => {
    // The dedicated `open-design/plugin-registry` repo per
    // docs/plans/plugin-registry.md §1.2 is the long-term target; until that
    // operational launch step happens, submissions land in `aresbit/autoagent-design`
    ...
    expect(link.catalogLabel).toBe('aresbit/autoagent-design');
    expect(link.url).toMatch(/^https:\/\/github\.com\/aresbit\/autoagent-design\/issues\/new\?/);
