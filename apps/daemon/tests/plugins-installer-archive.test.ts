...
describe('archive installer', () => {
  ...
  it('extracts a github source with a ref and plugin subpath', async () => {
    ...
    const apiUrl =
      'https://api.github.com/repos/aresbit/autoagent-design/contents/plugins/community/registry-starter?ref=garnet-hemisphere';
    ...
    let success = false;
    let error: string | undefined;
    const source = 'github:aresbit/autoagent-design@garnet-hemisphere/plugins/community/registry-starter';
    for await (const ev of installPlugin(db, {
      source,
      roots: { userPluginsRoot: pluginsRoot },
      fetcher,
    ...
    expect(urlsSeen).not.toContain('https://codeload.github.com/aresbit/autoagent-design/tar.gz/garnet-hemisphere');
    const row = db.prepare(`SELECT source_kind, source FROM installed_plugins WHERE id = 'sample-plugin'`).get();
  ...
  it.each([
    ...
    const urlsSeen: string[] = [];
    const contentsUrl =
      'https://api.github.com/repos/aresbit/autoagent-design/contents/plugins/community/import-smoke-test?ref=main';
    const tarballUrl = 'https://codeload.github.com/aresbit/autoagent-design/tar.gz/main';
    const fetcher: ArchiveFetcher = async (u) => {
      urlsSeen.push(u);
      if (u === contentsUrl) {
    ...
    let success = false;
    let error: string | undefined;
    const source = 'github:aresbit/autoagent-design@main/plugins/community/import-smoke-test';
    for await (const ev of installPlugin(db, {
      source,
      roots: { userPluginsRoot: pluginsRoot },
      fetcher,
