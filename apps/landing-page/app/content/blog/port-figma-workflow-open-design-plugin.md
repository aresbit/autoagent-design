---
title: "How to port a Figma workflow into an Auto Design plugin"
date: 2026-05-18
category: "Use cases"
readingTime: 6
...
That is why Auto Design's [skill layer](/blog/why-we-built-open-design-as-a-skill-layer/) matters. A plugin is not an opaque runtime extension. It is a folder of files: a `SKILL.md` contract, optional design systems, optional examples, and an `open-design.json` sidecar that tells Auto Design how to display and apply the workflow.

## The Auto Design angle is portability
...
That gives one workflow two lives. In Auto Design, it appears as a plugin with a preview, inputs, provenance, and a one-click "use" path. In Claude Code, Cursor, Codex, Gemini CLI, OpenClaw, or another skill catalog, the same folder still works as a plain agent skill because the core behavior lives in Markdown.
...
| Auto Design metadata | `open-design.json` |
...
3. **Add the Auto Design sidecar.** Create `open-design.json` so the marketplace can show the title, description, required inputs, preview, and source repo.
...
publishing docs for the current review path. Auto Design does not store
...
| Review section IDs, copy, density, and responsive behavior | Human in the Auto Design preview |
...
- [31 skills, 72 systems: how the Auto Design library works](/blog/31-skills-72-systems-how-the-library-works/) — the primitives a plugin wraps
- [Why we built Auto Design as a skill layer, not a product](/blog/why-we-built-open-design-as-a-skill-layer/) — why the workflow is file-shaped instead of account-shaped
