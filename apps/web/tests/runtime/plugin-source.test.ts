...
describe('derivePluginSourceLinks · url + local + bundled sources', () => {
  ...
  it('routes bundled official sources to the Auto Design repo', () => {
    const out = derivePluginSourceLinks(
      makeRecord({
        sourceKind: 'bundled',
    ...
    expect(out.sourceUrl).toBe('https://github.com/aresbit/autoagent-design');
    expect(out.sourceKindLabel).toBe('Official');
    expect(out.sourceLabel).toBe('aresbit/autoagent-design');
    expect(out.authorProfileUrl).toBe('https://github.com/aresbit/autoagent-design');
    expect(out.homepageUrl).toBe('https://github.com/aresbit/autoagent-design');
  });
});
    ...
    const out = derivePluginSourceLinks(
      makeRecord({
        manifest: {
          name:    'p',
          version: '1.0.0',
          author:  { name: 'Auto Design', url: 'https://github.com/nexu-io' },
        } as InstalledPluginRecord['manifest'],
      }),
    );
    expect(out.authorName).toBe('Auto Design');
    ...
    const out = derivePluginSourceLinks(
      makeRecord({
        ...
        manifest: {
          name:    'p',
          version: '1.0.0',
          homepage: 'https://github.com/aresbit/autoagent-design',
        } as InstalledPluginRecord['manifest'],
      }),
    );
    expect(out.contributeUrl).toBe('https://github.com/aresbit/autoagent-design/issues/new');
    expect(out.contributeOnGithub).toBe(true);
    expect(out.homepageUrl).toBe('https://github.com/aresbit/autoagent-design');
  });
...
describe('authorInitials', () => {
  it('builds two-letter monograms', () => {
    expect(authorInitials('Auto Design')).toBe('OD');
