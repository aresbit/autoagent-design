...
describe('SettingsDialog MCP server interactions', () => {
  const installInfo = {
    command: '/Applications/Auto Design.app/Contents/Resources/open-design/bin/node',
    args: [
      '/Applications/Auto Design.app/Contents/Resources/app/node_modules/@open-design/daemon/dist/cli.js',
      'mcp',
      '--daemon-url',
  ...
  it('renders the default Claude Code install snippet after fetching daemon install info', async () => {
    ...
    expect(screen.getByText(/Auto Design must be running for MCP tool calls to succeed/i)).toBeTruthy();
  });
...
describe('SettingsDialog about interactions', () => {
  ...
  it('opens the releases page when the latest release info is stale', async () => {
    ...
    fetchLatestGithubReleaseInfoMock.mockResolvedValue({
      tagName: 'v0.4.1',
      htmlUrl: 'https://github.com/aresbit/autoagent-design/releases/tag/v0.4.1',
      stale: true,
    });
    ...
    await waitFor(() => {
      expect(openSpy).toHaveBeenCalledWith(
        'https://github.com/aresbit/autoagent-design/releases',
        '_blank',
        'noopener,noreferrer',
      );
    });
