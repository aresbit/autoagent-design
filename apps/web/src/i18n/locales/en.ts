...
export const en: Dict = {
  ...
  'settings.onboardingCreateBody':
    'Describe the site, app, deck, image, or video you want. Auto Design will create a project and keep the work editable.',
  ...
  'settings.agentInstall.pathHint':
    'If you installed a CLI with npm or Homebrew and it still shows as not installed, ensure the tool\'s bin directory is on the PATH the Auto Design daemon inherits (Terminal vs GUI apps can differ on macOS). See QUICKSTART.md (section "Local agent CLI and PATH").',
  'settings.agentInstall.stepOpenLinks': 'Open Install or Docs for your preferred agent.',
  'settings.agentInstall.stepAuth':
    'Authenticate with the vendor CLI (sign in or add API credentials) before returning to Auto Design.',
  ...
  'settings.mcpServerHint': 'Expose Auto Design as an MCP server for your coding agent.',
  ...
  'settings.privacyHint': 'What data is shared with the Auto Design team',
  'settings.privacyConsentKicker': 'Help us improve Auto Design',
  'settings.privacyConsentLead': 'Auto Design can share usage data with our team to help us improve. This includes:',
  ...
  'settings.mcpHint': 'Let coding agents (Cursor, Claude Code, VS Code…) read your Auto Design projects directly.',
  'settings.mcpDaemonError':
    "Couldn't reach the local daemon to resolve install paths ({error}). Make sure Auto Design is running, then reopen this panel.",
  ...
  'settings.mcpCapabilityDefault':
    'Default to the project and file you have open in Auto Design, so you can say "build this in my app" without re-stating which design.',
  'settings.mcpRunningNote':
    'Auto Design must be running for MCP tool calls to succeed. If you started your coding agent before opening Auto Design, restart the agent so it can reach the live daemon.',
  ...
  'homeHero.chip.createPluginHint': 'Author a reusable Auto Design plugin and add it to My plugins.',
  ...
  'designSystemPicker.noPreview': 'No preview page. Auto Design Systems to view the full preview.',
  ...
  'pluginsHome.subtitle': 'Ready-to-use Auto Design workflows bundled with this runtime. Pick one to load a starter prompt, or browse the registry for more.',
  'pluginsHome.browseRegistry': 'Browse registry',
  'pluginsHome.count': '{filtered} of {total}',
  'pluginsHome.loadingCatalog': 'Loading catalog…',
  'pluginsHome.emptyCatalog': 'Catalog is empty. Bundled plugins ship with Auto Design and should appear here automatically — try restarting the daemon if this persists.',
  ...
  'pluginCard.contributeAria': 'Contribute {title} to Auto Design',
  'pluginCard.contributeTitle': 'Contribute plugin to Auto Design with a pull request',
  'integrations.kicker': 'Integration',
  'integrations.lede': 'Connect external systems, bring MCP tools into your agent loop, and use Auto Design from other IDEs, scripts, and automations.',
  ...
  'mcpClient.daemonError': 'Could not reach the local daemon. Make sure Auto Design is running, then reopen this panel.',
  'mcpClient.saveFailed': 'Save failed. Check that the daemon is running and try again.',
  'useEverywhere.modalAria': 'Use Auto Design everywhere',
  'useEverywhere.modalTitle': 'Use Auto Design everywhere',
  'useEverywhere.modalSubtitle': 'Drop Auto Design into any IDE, agent, or script — CLI, HTTP, MCP, and Skills. Use “Copy guide for an agent” and paste into Claude Code, Codex, Cursor, openclaw, or hermes to set up everything.',
  ...
  'useEverywhere.section.overview.heading': 'Auto Design works wherever your agent works',
  'useEverywhere.section.overview.intro': 'Auto Design is more than a window — it is a local privileged daemon (`od`) plus a Skills + Design-Systems + Atoms registry. Once it is running on your machine, any code agent (Claude Code, Codex, Cursor, OpenCode/openclaw, Hermes, your own script) can drive generations, inspect projects, and produce design artifacts through four interchangeable surfaces.',
  'useEverywhere.section.overview.bullet1': 'CLI — `od <command>` for headless scripts, CI, and shell automation.',
  'useEverywhere.section.overview.bullet2': 'MCP server — wires Auto Design as a Model Context Protocol server so any MCP-capable agent can list skills, run scenarios, and read artifacts.',
  ...
  'useEverywhere.section.overview.bullet4': 'Skills — drop-in `SKILL.md` packs (Claude-compatible) that any agent already on your PATH can invoke without Auto Design at all.',
  ...
  'useEverywhere.section.cli.heading': 'Drive Auto Design from any shell',
  ...
  'useEverywhere.section.mcp.heading': 'Expose Auto Design as an MCP server to any coding agent',
  'useEverywhere.section.mcp.intro': 'Auto Design ships with a Model Context Protocol server (`od mcp`) that lets any MCP-capable client — Cursor, Claude Code, Antigravity, VS Code Copilot Chat, openclaw, hermes — discover Auto Design tools (list skills, render previews, generate media, run plugins) without shelling out manually. The daemon publishes a ready-to-paste install snippet via `GET /api/mcp/install-info` for each major client.',
  ...
  'useEverywhere.section.mcp.footer': 'In the Auto Design app, open Settings → Integrations to copy a client-specific install command (Cursor, Claude Code, Antigravity, VS Code) instead of editing JSON by hand.',
  ...
  'useEverywhere.section.skills.heading': 'Drop-in Skills for any agent — even without Auto Design running',
  'useEverywhere.section.skills.intro': 'A Skill is a directory with a Claude-compatible `SKILL.md` (YAML front-matter + body). Auto Design extends the format with the `od:` namespace (`mode`, `preview`, `design_system`, `inputs`, …) so the same artifact can be used both inside Auto Design and by a vanilla agent like Claude Code, Codex, openclaw, or hermes. Discovery follows a precedence chain so projects can override their own skills.',
  ...
  'useEverywhere.section.skills.snippet1': 'Minimal SKILL.md (Claude-compatible front matter + Auto Design extras)',
  ...
  'entry.githubStarAria': 'Star Auto Design on GitHub',
  ...
  'preview.errorBody': 'The example HTML failed to fetch. Make sure Auto Design is running and try again.',
  'preview.retry': 'Retry',
  ...
  'pet.tabBuiltInHint': 'Curated companions bundled with Auto Design — pick one and adopt.',
  ...
  'pet.codexBundledTitle': 'Ships with Auto Design — no download needed.',
  ...
  'settings.memoryStarterProjectDesc': 'Auto Design v0.5 — chat-driven design editor',
  ...
  'updater.installerOpenBody': 'The installer is open. Quit Auto Design before replacing the app.',
  ...
  'updater.quitButton': 'Quit Auto Design',
  'updater.quitFailedBody': 'Auto Design could not quit.',
  ...
  'updater.readyVersion': 'Auto Design {version} is ready.',
