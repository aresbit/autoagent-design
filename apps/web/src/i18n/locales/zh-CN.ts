...
export const zhCN: Dict = {
  ...
  'settings.onboardingCreateBody':
    '描述你想做的网站、应用、Deck、图片或视频，Auto Design 会创建项目，并保留可继续编辑的产物。',
  ...
  'settings.agentInstall.pathHint':
    '如果你通过 npm 或 Homebrew 安装了 CLI，但仍显示为未安装，请确认该工具的 bin 目录已加入 Auto Design daemon 继承的 PATH（在 macOS 上，Terminal 与 GUI 应用的 PATH 可能不同）。请参阅 QUICKSTART.md（“Local agent CLI and PATH” 章节）。',
  'settings.agentInstall.stepOpenLinks': '在目标代理卡片上打开“安装”或“文档”链接。',
  'settings.agentInstall.stepAuth':
    '返回 Auto Design 之前，请先在对应 CLI 中完成认证（登录或添加 API 凭据）。',
  ...
  'settings.mcpServerHint': '将 Auto Design 作为 MCP 服务器暴露给你的编码代理。',
  ...
  'settings.privacy': '隐私',
  'settings.privacyHint': '与 Auto Design 团队共享哪些数据',
  'settings.privacyConsentKicker': '帮助我们改进 Auto Design',
  'settings.privacyConsentLead': 'Auto Design 可以将使用数据共享给我们的团队以协助改进。包括:',
  ...
  'settings.mcpHint':
    '让其他仓库中的编码助手（Claude Code、Cursor、VS Code、Antigravity、Zed、Windsurf）读取你的 Auto Design 项目。无需先导出 zip，即可将设计拉取到你的应用中。',
  'settings.mcpDaemonError':
    '无法连接到本地守护进程以解析安装路径（{error}）。请确保 Auto Design 正在运行，然后重新打开此面板。',
  ...
  'settings.mcpCapabilityDefault':
    '默认使用你在 Auto Design 中打开的项目和文件，因此你可以直接说"在我的应用中构建这个"，无需重复说明是哪个设计。',
  'settings.mcpRunningNote':
    'Auto Design 必须处于运行状态，MCP 工具调用才能成功。如果你在打开 Auto Design 之前启动了编码助手，请重启助手以便它能连接到正在运行的守护进程。',
  ...
  'homeHero.chip.createPluginHint': '创作可复用的 Auto Design 插件，并添加到我的插件。',
  ...
  'pluginsHome.subtitle': '当前运行环境内置的 Auto Design 工作流。选择一个加载 starter prompt，或浏览插件市场查看更多。',
  'pluginsHome.browseRegistry': '浏览插件市场',
  'pluginsHome.count': '{filtered} / {total}',
  'pluginsHome.loadingCatalog': '正在加载目录…',
  'pluginsHome.emptyCatalog': '目录为空。Auto Design 会随附内置插件，如果这里一直为空，请尝试重启 daemon。',
  ...
  'pluginCard.contributeAria': '将 {title} 贡献到 Auto Design',
  'pluginCard.contributeTitle': '通过 pull request 将插件贡献到 Auto Design',
  'integrations.kicker': '集成',
  'integrations.lede': '连接外部系统，把 MCP 工具带入智能体循环，并在其它 IDE、脚本和自动化中使用 Auto Design。',
  ...
  'mcpClient.daemonError': '无法连接本地 daemon。请确认 Auto Design 正在运行，然后重新打开此面板。',
  'mcpClient.saveFailed': '保存失败。请检查 daemon 是否正在运行后重试。',
  'useEverywhere.modalAria': '随处使用 Auto Design',
  'useEverywhere.modalTitle': '随处使用 Auto Design',
  'useEverywhere.modalSubtitle': '把 Auto Design 接入任何 IDE、智能体或脚本：CLI、HTTP、MCP 和 Skills。使用“复制智能体指南”，粘贴到 Claude Code、Codex、Cursor、openclaw 或 hermes 中即可完成配置。',
  ...
  'useEverywhere.section.overview.heading': 'Auto Design 可在智能体工作的任何地方运行',
  'useEverywhere.section.overview.intro': 'Auto Design 不只是一个窗口；它包含本地特权 daemon（`od`），以及 Skills + Design Systems + Atoms registry。它在你的机器上运行后，任何代码智能体（Claude Code、Codex、Cursor、OpenCode/openclaw、Hermes 或你自己的脚本）都可以通过四种可互换入口驱动生成、检查项目并产出设计制品。',
  'useEverywhere.section.overview.bullet1': 'CLI — 使用 `od <command>` 支持无头脚本、CI 和 shell 自动化。',
  'useEverywhere.section.overview.bullet2': 'MCP 服务器 — 将 Auto Design 作为 Model Context Protocol 服务器接入，使任何支持 MCP 的智能体都能列出技能、运行场景并读取制品。',
  'useEverywhere.section.overview.bullet3': 'HTTP API — `http://127.0.0.1:7456/api/*` REST + SSE 端点；与 Web UI 使用同一套接口。',
  'useEverywhere.section.overview.bullet4': 'Skills — 可直接放入的 `SKILL.md` 包（兼容 Claude），任何已在 PATH 中的智能体都能调用，甚至不需要打开 Auto Design。',
  ...
  'useEverywhere.section.cli.heading': '从任意 shell 驱动 Auto Design',
  ...
  'useEverywhere.section.mcp.heading': '把 Auto Design 暴露为任意代码智能体可用的 MCP 服务器',
  'useEverywhere.section.mcp.intro': 'Auto Design 内置 Model Context Protocol 服务器（`od mcp`），让任何支持 MCP 的客户端（Cursor、Claude Code、Antigravity、VS Code Copilot Chat、openclaw、hermes）都能发现 Auto Design 工具（列出技能、渲染预览、生成媒体、运行插件），无需手动 shell 调用。daemon 会通过 `GET /api/mcp/install-info` 为主流客户端发布可直接粘贴的安装片段。',
  ...
  'useEverywhere.section.mcp.footer': '在 Auto Design 应用中，打开“设置 → 集成”即可复制特定客户端的安装命令（Cursor、Claude Code、Antigravity、VS Code），无需手写 JSON。',
  ...
  'useEverywhere.section.skills.heading': '任意智能体可直接使用的 Skills，即使 Auto Design 未运行',
  'useEverywhere.section.skills.intro': 'Skill 是一个包含 Claude 兼容 `SKILL.md`（YAML front matter + 正文）的目录。Auto Design 通过 `od:` 命名空间（`mode`、`preview`、`design_system`、`inputs` 等）扩展该格式，使同一个制品既能在 Auto Design 内使用，也能被 Claude Code、Codex、openclaw 或 hermes 这类普通智能体使用。发现逻辑遵循优先级链，因此项目可以覆盖自己的 skills。',
  ...
  'useEverywhere.section.skills.snippet1': '最小 SKILL.md（Claude 兼容 front matter + Auto Design 扩展）',
  ...
  'entry.githubStarAria': '在 GitHub 上为 Auto Design 点亮 Star',
  ...
  'preview.errorBody': '示例 HTML 加载失败。请确认 Auto Design 正在运行后重试。',
  'preview.retry': '重试',
  ...
  'pet.title': '宠物',
  'pet.tabBuiltIn': '内置',
  'pet.tabBuiltInHint': 'Auto Design 内置的精选宠物 — 一键领养。',
  ...
  'pet.codexBundled': '内置',
  'pet.codexBundledTitle': 'Auto Design 内置宠物，无需下载。',
  ...
  'settings.memoryStarterProjectDesc': 'Auto Design v0.5 — 聊天驱动的设计编辑器',
