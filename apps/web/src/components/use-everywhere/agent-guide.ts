...
// everything it needs to install Auto Design, expose it as MCP, and
...
export interface AgentGuideOptions {
   ...
   */
  cliHint?: string;
  /** Optional Auto Design version/channel; surfaced in the header for support tickets. */
  versionHint?: string;
}
...
export function buildAgentGuideMarkdown(options: AgentGuideOptions = {}): string {
  ...
  lines.push('# Auto Design — agent setup guide');
  lines.push('');
  lines.push(
    'You are a coding agent (Claude Code, Codex, Cursor, openclaw, hermes, or similar). ' +
      'The user wants Auto Design wired into your workflow. Auto Design is a local ' +
  ...
  if (options.versionHint) {
    lines.push('');
    lines.push(`> Reported Auto Design version: \`${options.versionHint}\``);
  }
  lines.push('');
  ...
  lines.push('   ```');
  lines.push('');
  lines.push('   If it 404s or times out, ask the user to run `pnpm tools-dev` (dev) or open the Auto Design app (packaged).');
  lines.push('');
