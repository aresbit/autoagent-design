...
// `contribute` opens a draft PR against the `aresbit/autoagent-design` community
...
function buildContributePrompt(folderPath: string): string {
  return [
    'Open a draft Pull Request that adds this generated plugin to the Auto Design community catalog at `aresbit/autoagent-design`.',
    ...
    '',
    '4. **Fork the registry repo.** Run `gh repo fork aresbit/autoagent-design --remote=false`. Tolerate "already exists" / "existing fork" — it is idempotent.',
    '',
...
// Auto Design issue URL and belongs to the "Auto Design PR" button. Before
...
function buildPublishPrompt(folderPath: string): string {
  return [
    ...
    `Manifest: \`${folderPath}/open-design.json\``,
    '',
    'This is the **repository publish** action, NOT the registry-submission action — do NOT route through `od plugin publish --to open-design`. That command emits an Auto Design issue URL and belongs to the "Auto Design PR" button.',
    '',
    'Run the steps below in order. Report each command and its result. Stop on the first hard failure — do not retry blindly.',
    '',
