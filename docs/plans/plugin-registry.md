# Auto Design Plugin Registry — Plan (living)
...
> up their own Auto Design plugin source with one config line.
...
start in the main Auto Design repo, but the code path must still be expressed as
...
- `od plugin login` wraps `gh auth login` with Auto Design copy, scopes, and
