...
- Local packaging orchestration for packaged Auto Design artifacts.
...
- Public release artifacts must use channel-specific app identity: stable uses `Auto Design`, beta uses `Auto Design Beta`, and preview uses `Auto Design Preview`. Local tools-pack installs may still use namespace-scoped install paths only as a developer multi-instance validation convention.
...
- Stable: `Auto Design`, namespace `default` or stable release namespace.
- Beta Windows: `Auto Design Beta`, namespace `release-beta-win`, uninstall key `Auto Design-release-beta-win`.
- Preview Windows: `Auto Design Preview`, namespace `release-preview-win`, uninstall key `Auto Design-release-preview-win`.
...
```text
C:\odtp-beta-release-fixed\out\win\namespaces\release-beta-win\builder\Auto Design-release-beta-win-setup.exe
```

4. Expected user flow:

- User installs `0.8.0-beta.5` through the NSIS UI.
- User launches `Auto Design Beta`.
...
- Clicking `Open installer` opens the real downloaded beta installer. Installing it should overwrite the same `Auto Design-release-beta-win` registry key, not create a second beta key.

5. Registry sanity check after beta.6 install:

```powershell
Get-ItemProperty 'HKCU:\Software\Microsoft\Windows\CurrentVersion\Uninstall\*' -ErrorAction SilentlyContinue |
  Where-Object { $_.DisplayName -like 'Auto Design*' } |
  Select-Object PSChildName,DisplayName,DisplayVersion,InstallLocation
```

For a clean beta channel result, expect one beta entry with `PSChildName` `Auto Design-release-beta-win` and the latest `DisplayVersion`.
