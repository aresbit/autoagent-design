...
async function runFixture(options: {
  ...
  const resourcesRoot = platformName === "darwin"
    ? join(appOutDir, "Auto Design.app", "Contents", "Resources")
    : join(appOutDir, "resources");
  const appPath = join(appOutDir, "Auto Design.app");
  ...
  if (platformName === "darwin" && options.writeMacCodeBundleFixture) {
    ...
    await mkdir(join(frameworksRoot, "Auto Design Helper.app"), { recursive: true });
  }
  ...
  try {
    await runWebStandaloneAfterPack({
      ...
      packager: { appInfo: { productFilename: "Auto Design" } },
    });
  } catch (error) {
    ...
    try {
      ...
      expect(signedTargets).toEqual(
        expect.arrayContaining([
          ...
          expect.stringMatching(/Auto Design Helper\.app$/),
          expect.stringMatching(/Auto Design\.app$/),
        ]),
      );
