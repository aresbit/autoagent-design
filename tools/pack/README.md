...
Local packaging control plane for Auto Design.
...
Release artifacts keep the canonical `Auto Design.app` bundle shape; local `tools-pack install` copies it as
`Auto Design.<namespace>.app` so developer namespaces can coexist without affecting runtime data/log/cache paths.
...
- AppImage: `~/.local/bin/Auto-Design.<namespace>.AppImage`
