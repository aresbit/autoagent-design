...
Auto Design plugins are intentionally shaped so one folder can travel across multiple agent ecosystems. The safest publishing model is:

1. Keep the source of truth in a public GitHub repository or in an Auto Design PR.
2. Keep `SKILL.md` portable and registry-friendly.
3. Add `open-design.json` as the Auto Design sidecar.
...
2. Push a public GitHub repository or open an Auto Design PR.
3. Add README install instructions for Auto Design and generic Agent Skills clients.
...
| --- | --- | --- | --- |
| Auto Design | OD marketplace, composer chips, pipelines, GenUI, artifact provenance | `SKILL.md` + `open-design.json` | Open a PR to Auto Design or publish a marketplace index entry pointing to the plugin repo. |
...
For Auto Design plugin authors:
...
Use this path for normal Auto Design plugins because they are centered on `SKILL.md`.
...
- Link back to the canonical GitHub repo and Auto Design PR.
